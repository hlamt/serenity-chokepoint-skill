...
## Example 8｜AI liquid cooling chokepoint scan
...
Treat the task as an operational chokepoint scan. Define the downstream buildout and throughput, reverse-engineer the chain by layer, distinguish dependency / bottleneck / chokepoint / TOC-style constraint, and keep all companies as research candidates rather than recommendations.

### Expected output skeleton

- 研究问题: whether AI data center liquid cooling contains candidate supply-chain chokepoints.
- Downstream buildout / Throughput: stable deployment of high-density AI racks, not merely component sales.
- 价值链地图: heat source, cold plate, manifold, quick disconnect, pump/valve, CDU, coolant, facility-side heat rejection, controls, integration, qualification, and service.
- 候选层级: cold plate design, quick-disconnect reliability, CDU/control systems, facility integration, and customer qualification as hypotheses only.
- Dependency / Bottleneck / Chokepoint / TOC constraint: explain what each liquid-cooling layer currently proves and what it does not prove.
- Validation signals: customer qualification, design win, lead-time expansion, LTA/prepayment, shortage language, pricing/ASP, field reliability, M&A or vertical integration interest.
- Invalidation signals: design-out, competing capacity, alternative cooling path, qualification failure, materiality too small, weak firm capture, weak shareholder capture, demand timing miss, thesis fully priced in.
- Firm Value Capture: test whether the supplier can retain economics from the layer.
- Shareholder Capture: separately test dilution, valuation, governance, capital structure, timing, and per-share economics.
- 非投资建议声明: no buy/sell/hold, target price, position sizing, or investment conclusion.

### Prohibited output

- Buy list or investment ranking.
- Candidate layer equals confirmed chokepoint.
- Supplier participation equals control-point power.
- Firm value capture equals shareholder capture.
- Treating one media report, customer rumor, or shortage narrative as proof.

## Example 9｜SIVE / CW laser / ELS case pattern
...
Do not directly confirm. Explain that CW laser / ELS may sit in a candidate CPO / 1.6T optical-interconnect control layer, then test qualification, foundry allocation, customer proof, design-out risk, materiality, firm value capture, and shareholder capture.

### Expected output skeleton

- Direct answer: 不能直接认定；这里只能先把 SIVE 作为 CPO / CW laser / ELS case pattern 的研究候选。
- Layer position: CW laser / external light source may matter if CPO or 1.6T optical interconnects require scarce, qualified light-source capacity.
- Control-point tests: concentration, allocation control, foundry/epiwafer dependency, qualification cycle, customer design win, lead time, and substitution difficulty.
- False-positive checks: participation in a relevant photonics layer does not prove control-point power.
- Design-out risk: alternative optical architecture, vertical integration by larger players, different laser path, or customer insourcing.
- Materiality: test whether the relevant revenue can become large enough to affect firm economics.
- Firm value capture: pricing power, margin, bargaining power, volume ramp, and where economics accrue.
- Shareholder capture: valuation, dilution, governance, capital structure, liquidity, and timing.
- 非投资建议声明: no recommendation, no target price, no position sizing.

### Prohibited output

- "SIVE is definitely the chokepoint."
- "SIVE is a buy."
- Treating Grok synthesis, public commentary, or a single OSINT thread as primary proof.
- Company participation equals durable control.

## Example 10｜Firm value capture vs shareholder capture

### User input

如果一家公司真的控制了供应链卡点，是不是股东一定会赚到？

### Skill should do

Answer no. Separate firm value capture from shareholder capture and give a verification checklist. Keep the answer non-advice and avoid price, timing, return, or allocation conclusions.

### Expected output skeleton

- Direct answer: 不一定。
- Firm Value Capture: the firm may capture economics through pricing, margin, allocation control, LTAs, customer dependence, or strategic scarcity.
- Shareholder Capture: current public shareholders only benefit if per-share economics survive dilution, valuation, governance, capital structure, capex, liquidity, and timing.
- Verification checklist: share count, financing terms, debt, preferred securities, warrants, insider control, capex, working capital, margins, customer concentration, valuation expectations, and evidence that economics reach common shareholders.
- Invalidation conditions: dilution absorbs gains, financing transfers value away, capex consumes returns, governance blocks minority capture, demand arrives too late, or the thesis is already fully priced in.
- 非投资建议声明: no security selection, no target price, no position sizing, no return promise.

### Prohibited output

- Company value capture means shareholders win.
- Dilution is automatically good or bad.
- Any investment conclusion.

## Example 11｜Direct stock recommendation boundary
...
Refuse the direct buy recommendation and redirect to a candidate research universe. The answer may group companies by value-chain layer, control-point hypothesis, and evidence strength, but must not give buy/sell/hold, target price, position sizing, or investment ranking.

### Expected output skeleton

- Refusal: 不能直接告诉你现在应该买哪几只；这会变成具体买入建议。
- Redirect: 可以改成 CPO 供应链候选研究池。
- Candidate grouping: substrates/materials, CW laser / ELS, optical interposer, transceiver / module integration, foundry/epiwafer, testing, hyperscaler/customer qualification.
- For each group: possible control-point hypothesis, evidence needed, validation signals, invalidation signals, firm value-capture risk, shareholder-capture risk.
- Boundary: following order is value-chain grouping or evidence strength, not investment attractiveness.
...
- Best-stock list.
- Winner ranking.
- "top picks" as investment recommendations.
